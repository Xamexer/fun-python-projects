...
class Game:
    def __init__(self) -> None:
        ...
        self.gameWon = False
        winsound.PlaySound(".\\mind.wav", winsound.SND_ASYNC | winsound.SND_ALIAS | winsound.SND_LOOP )
    ...
    def drawScreen(self):
        ...
        if self.gameRunning:
            self.elapsed_time = int(time.time() - self.start_time) + self.old_elapsed_time
        else:
        ...
        if not self.gameRunning:
            if self.gameWon:
                self.screen.addstr(7+MAX_LIVES*2,10,f'GAME WON !',self.COLOR_GREEN)
            else:
                self.screen.addstr(5+MAX_LIVES*2,3,f'Word to guess was "{self.wordToGuess}"')
                self.screen.addstr(7+MAX_LIVES*2,10,f'GAME LOST !',self.COLOR_RED)

            self.screen.addstr(8+MAX_LIVES*2,6,f'enter to continue.')
